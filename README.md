...
## GitHub Actions 一键部署（无需手动进入 Cloudflare 后台）

仓库已包含 `.github/workflows/deploy.yml`。你只需要在 GitHub 仓库后台配置 Secrets/Variables，之后推送到 `main` 或手动运行 workflow 即可自动完成：

1. 安装依赖并运行 TypeScript 检查。
2. 调用 Cloudflare API 自动创建或复用 KV namespace。
3. 调用 Cloudflare API 自动创建或复用 R2 bucket。
4. 生成 `wrangler.generated.toml`。
5. 使用生成的配置部署 Worker。
...
| Secret | 说明 |
| --- | --- |
| `CLOUDFLARE_API_TOKEN` | Cloudflare API Token。建议授予 Account KV Storage Edit、Account R2 Storage Edit、Workers Scripts Edit 权限。 |
| `CLOUDFLARE_ACCOUNT_ID` | Cloudflare Account ID。 |

### 可选 GitHub Variables

在 GitHub 仓库：`Settings` → `Secrets and variables` → `Actions` → `Variables` 中可选添加：

| Variable | 默认值 | 说明 |
| --- | --- | --- |
| `CLOUDFLARE_WORKER_NAME` | `friends-climbing` | Worker 名称。 |
| `CLOUDFLARE_KV_NAMESPACE` | `friends-climbing-kv` | 生产 KV namespace 名称。 |
| `CLOUDFLARE_PREVIEW_KV_NAMESPACE` | `friends-climbing-kv-preview` | Preview KV namespace 名称。 |
| `CLOUDFLARE_R2_BUCKET` | `friends-climbing-images` | 生产 R2 bucket 名称。 |
| `CLOUDFLARE_PREVIEW_R2_BUCKET` | `friends-climbing-images-dev` | Preview R2 bucket 名称。 |

### API Token 权限建议

为了让 GitHub Actions 能在不操作 Cloudflare 后台的情况下自动创建资源并部署，Token 至少需要：
...
如果你还希望后续扩展自定义域名/路由自动配置，可额外给 Zone 相关权限；当前 workflow 默认部署到 `*.workers.dev`，不要求手动配置 Cloudflare 后台。

### Workflow 文件

- `.github/workflows/deploy.yml`：CI/CD 流程。
- `scripts/cloudflare-resources.mjs`：通过 Cloudflare API 自动确保 KV/R2 资源存在，并生成临时 `wrangler.generated.toml`。

`wrangler.generated.toml` 不需要提交，它只在 GitHub Actions 运行时生成。
