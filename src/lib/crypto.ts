...
export async function hashPassword(password: string): Promise<PasswordHash> {
  ...
  const bits = await crypto.subtle.deriveBits(
    { name: 'PBKDF2', hash: 'SHA-256', salt, iterations: 524288 },
    key,
    512,
  );
  return {
    ...
    iterations: 524288,
    salt: b64u(salt),
    derivedKey: b64u(bits),
    derivedKeyLength: 64,
  };
}
export async function verifyPassword(password: string, h: PasswordHash) {
  const key = await crypto.subtle.importKey('raw', te.encode(password), 'PBKDF2', false, [
    'deriveBits',
  ]);
  const bits = new Uint8Array(
    await crypto.subtle.deriveBits(
      { name: 'PBKDF2', hash: 'SHA-256', salt: ub64(h.salt), iterations: h.iterations },
      key,
      h.derivedKeyLength * 8,
    ),
  );
  return timingSafeEqual(bits, ub64(h.derivedKey));
}
